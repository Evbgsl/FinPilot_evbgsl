...
public class ReportArgsParser {
    ...
    public static ReportFilter parse(String line, String basePrefix) {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (t.equals("--from")) {
                ...
                if (only.isEmpty()) {
                    throw new IllegalArgumentException("Параметр --only не должен быть пустым. Пример: --only food,taxi");
                }
            } else {
    ...
    private static LocalDate parseDate(String s, String flag) {
        try {
            ...
            throw new IllegalArgumentException("Некорректная дата для " + flag + ": " + s + " (ожидается yyyy-mm-dd)");
        }
    }
