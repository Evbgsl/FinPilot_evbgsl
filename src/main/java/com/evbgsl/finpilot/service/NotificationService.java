...
public class NotificationService {
        ...
        .ifPresent(
            budget -> {
              ...
              if (crossed80Percent(budget, spentBefore, spentAfter)) {
                notifications.add(
                    "Предупреждение: вы достигли 80% бюджета по категории '"
                        + budget.category()
                        + "'. Потрачено: "
                        + spentAfter
                        + " из "
              ...
              if (crossedLimit(budget, spentBefore, spentAfter)) {
                ...
                notifications.add(
                    "Лимит превышен по категории '" + budget.category() + "'. Перерасход: " + over);
              }
            });
  ...
  private boolean crossed80Percent(Budget budget, Money before, Money after) {
    ...
    return before.value().compareTo(threshold) < 0
        && after.value().compareTo(threshold) >= 0
