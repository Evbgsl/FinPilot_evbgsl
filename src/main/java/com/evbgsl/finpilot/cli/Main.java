...
public class Main {
  public static void main(String[] args) {
    ...
    System.out.println(
        "FinPilot CLI запущен. Введите 'help' для отображения команд, 'exit' для выхода.");
    ...
    while (true) {
      ...
      try {
        if (line.startsWith("register ")) {
          ...
          for (var r : rows) {
            tableRows.add(
                java.util.List.of(r.category(), r.income().toString(), r.expense().toString()));
          }

          TablePrinter.print(java.util.List.of("Категория", "Доход", "Расход"), tableRows);
          ...
          if (rows.isEmpty()) {
            System.out.println(
                "Нет данных по бюджетам (или бюджеты не заданы / фильтры отфильтровали всё)");
            continue;
          }
          ...
          TablePrinter.print(
              java.util.List.of("Категория", "Лимит", "Потрачено", "Остаток"), tableRows);
          ...
          System.out.println(
              "Перевод выполнен: " + fromLogin + " -> " + toLogin + " (" + amount + ")");
          ...
          var imported =
              walletStorage
                  .load(path)
                  .orElseThrow(
                      () -> new IllegalArgumentException("Файл не найден или пустой: " + path));
          ...
          if (!importedLogin.equals(currentLogin)) {
            System.out.println(
                "Ошибка: кошелёк в файле принадлежит '"
                    + importedLogin
                    + "', а вы залогинены как '"
                    + currentLogin
                    + "'");
          ...
          authService.ensureLoggedIn();
          authService.getCurrentWallet().getOperations().forEach(System.out::println);
        } else {
  ...
  private static java.util.Set<String> knownCategoriesFromOperations(
      com.evbgsl.finpilot.core.Wallet wallet) {
  ...
  public static void printHelp() {
    ...
    System.out.println("Категории:");
    System.out.println(
        "  category add <name>               - добавить категорию (можно с пробелами)");
